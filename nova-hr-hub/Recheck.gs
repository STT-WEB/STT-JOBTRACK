...
function rcCalc_(inRaw, outRaw, dateStr, isMonthly, calMap, lunchApproved, dayType0) {
  ...
  /* ★★ ออกก่อนเข้า — ห้ามเดาว่า "ข้ามเที่ยงคืน" มั่ว ๆ
     ของเดิมเจอ o0 < i0 ปุ๊บบวก 1440 ทันที เคสจริง 26/8/2569 :
       เข้า 12:45 ออก 12:00  →  กลายเป็น 23 ชม. ได้ OT ×1.5 ฟรี 15 ชม.
  ...
  /* ★ ยาวเกินจริง = ลืมสแกนออก ไม่ใช่ทำงานจริง ห้ามคิดชั่วโมงให้เงียบ ๆ */
  var durChk = (o0 < i0 ? o0 + 1440 : o0) - i0;
  if (durChk > RC.MAX_SHIFT * 60) {
    r.err = 'ยาว ' + (durChk / 60).toFixed(1) + ' ชม. — น่าจะลืมสแกนออก ให้แก้เวลาออกช่อง D';
    return r;
  }
...
/* ============================================================================
 *  ★★ ส่วนกลาง — ใช้ร่วมกันทั้ง JOBTRACK และ Hub
 *
 *  ไฟล์นี้ถูกส่งขึ้นทั้ง 2 โปรเจกต์ (ต้นฉบับมีไฟล์เดียว ก๊อปตอน deploy)
 *  เพื่อให้ "เครื่องคิดเลขมีเครื่องเดียว" ทั้งระบบ
 *  ห้ามก๊อปโค้ดส่วนนี้ไปวางที่อื่น ถ้าจะแก้กฎ ให้แก้ที่ไฟล์นี้ที่เดียว
 * ========================================================================== */

/* ตำแหน่งคอลัมน์ผลคำนวณ (1-based) */
var RC_AC = { IN:25, OUT:26, AM:27, PM:28, WORK:29,
              OT1:30, OT15:31, OT2:32, OT3:33, TICK:34, STAT:35, NOTE:36,
              HOURTYPE:21, E:5, V:22, W:23, X:24 };
...
/** ป้ายประเภทชั่วโมง คอลัมน์ U — ถ้อยคำชุดเดียวกับที่ JOBTRACK เคยใช้ */
function rcHourType_(r, dayTypeVal) {
  if (!r || r.err) return '-';
  var hol = (rcKind_(dayTypeVal) === 'นักขัตฤกษ์') ? '2B' : '2A';
  var got = {};
  if (r.hNormal > 0)          got['1'] = true;
  if (r.ot1 > 0 || r.ot2 > 0) got[hol] = true;
  if (r.ot15 > 0)             got['3'] = true;
  if (r.ot3 > 0)              got['4'] = true;
  var parts = [];
  ['1', '2A', '2B', '3', '4'].forEach(function (c) { if (got[c]) parts.push(RC_HTYPE[c]); });
  return parts.join(' + ') || '-';
}

/** ข้อความสถานะ คอลัมน์ AI — บอกทุกเรื่องของแถวนั้น ไม่ใช่เรื่องเดียวแล้วกลบที่เหลือ */
function rcStatus_(r, n, pCol, collide, tick) {
...
/**
 *  เขียนผลคำนวณลงแถวเดียว — ทางเดียวที่ระบบเขียนผลลัพธ์
 *  ทั้ง JOBTRACK (ตอนสแกนออก) และ Hub (ตอน HR แก้เวลา) เรียกตัวนี้เหมือนกัน
 *  ผลจึงออกมาเหมือนกันเป๊ะเสมอ ไม่มีทางเพี้ยนกันได้อีก
 */
function rcWriteRow_(sheet, row, r, n, dayTypeVal, statusText) {
  if (n < 1) n = 1;
  var H = function (m) { return rcDec_(m / n); };
  var amH = H(r.amOT + r.amWork);
  var pmH = H(r.pmWork + r.pmOT + r.lunchOT);

  sheet.getRange(row, RC_AC.IN, 1, 9).setValues([[
    r.err ? '' : r.inUse, r.err ? '' : r.outUse,
    amH, pmH, Math.round((amH + pmH) * 100) / 100,
    H(r.ot1), H(r.ot15), H(r.ot2), H(r.ot3)
  ]]);
  sheet.getRange(row, RC_AC.STAT).setValue(statusText);
  sheet.getRange(row, RC_AC.HOURTYPE).setValue(rcHourType_(r, dayTypeVal));

  /* E · V · W · X เป็นสูตรเสมอ ห้ามเป็นเลขตาย */
  sheet.getRange(row, RC_AC.E).setNumberFormat('0.00')
       .setFormula('=IF($Q' + row + '<>"Check Out","",AC' + row + ')');
  sheet.getRange(row, RC_AC.V).setNumberFormat('0.00')
